...
@RestController
//@CrossOrigin({ "http://fastapi-server:8000" , "http://localhost:3000" })
@CrossOrigin({ "http://13.125.152.188:8000" , "http://13.125.152.188:3000" , "http://13.125.152.188" })
public class ChatController {
    ...
    public ChatController() {
        this.restClient = RestClient.builder()
                ...
                .baseUrl("http://13.125.152.188r:8000")
                .build();
    }

    @GetMapping("/test")
    public String index(){
        return "<h1>WELCOME CICD WORLD!!2222</h1>";
    }
    ...
    public ResponseEntity<byte[]> download(  @PathVariable String fileName) {
        ResponseEntity<byte[]> response = restClient.get()
                .uri("http://13.125.152.188:8000/download/" + fileName)
                .retrieve()
                .toEntity(byte[].class);
